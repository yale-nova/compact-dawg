...
class Dictionary {
 public:
  ...
  // Finds all keys lexicographically between start_key and end_key.
  void RangeSearch(const char *start_key, const char *end_key,
                   std::vector<std::string> *results) const {
    std::string key;
    RangeSearch(root(), start_key, end_key, 0, true, true, &key, results);
  }

  // Recursive helper for RangeSearch.
  void RangeSearch(BaseType index, const char *start_key, const char *end_key,
                   SizeType depth, bool start_bound, bool end_bound,
                   std::string *key, std::vector<std::string> *results) const {
    if (has_value(index)) {
      if (!start_bound || start_key[depth] == '\0') {
        results->push_back(*key);
      }
    }

    int min_label = start_bound ? static_cast<unsigned char>(start_key[depth]) : 0;
    int max_label = end_bound ? static_cast<unsigned char>(end_key[depth]) : 255;

    for (int i = min_label; i <= max_label; ++i) {
      UCharType label = static_cast<UCharType>(i);

      BaseType next_index = index ^ units_[index].offset() ^ label;

      if (next_index < static_cast<BaseType>(size_) && units_[next_index].label() == label) {
        key->push_back(static_cast<char>(label));
        RangeSearch(next_index, start_key, end_key, depth + 1,
                    start_bound && (i == min_label),
                    end_bound && (i == max_label),
                    key, results);
        key->pop_back();
      }
    }
...
public:
  ...
  void print_units() const {
    for (SizeType i = 0; i < size_; ++i) {
      std::cout << "Unit Label: " << units_[i].value() << std::endl;
                // << "Label: " << static_cast<int>(units_[i].label())
                // << ", Offset: " << units_[i].offset()
                // << ", Has Leaf: " << units_[i].has_leaf()
                // << ", Value: " << units_[i].value() << std::endl;
    }
  }
