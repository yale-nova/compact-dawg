...
#define GETBIT(n, i) ((((n) >> (i)) & 1ULL) != 0ULL)
#define SETBIT(n, i) n = (n | (1ULL << i))
#define CLRBIT(n, i) n = (n & ~(1ULL << i))

#define BITS2BLOCKS(bits) (((bits) % 64 == 0) ? ((bits) / 64) : (((bits) / 64) + 1))
...
static const uint64_t high_bits_unset[65] = {
    0xFFFFFFFFFFFFFFFFULL, 0x7FFFFFFFFFFFFFFFULL, 0x3FFFFFFFFFFFFFFFULL, 0x1FFFFFFFFFFFFFFFULL,
    0x0FFFFFFFFFFFFFFFULL, 0x07FFFFFFFFFFFFFFULL, 0x03FFFFFFFFFFFFFFULL, 0x01FFFFFFFFFFFFFFULL,
    0x00FFFFFFFFFFFFFFULL, 0x007FFFFFFFFFFFFFULL, 0x003FFFFFFFFFFFFFULL, 0x001FFFFFFFFFFFFFULL,
    0x000FFFFFFFFFFFFFULL, 0x0007FFFFFFFFFFFFULL, 0x0003FFFFFFFFFFFFULL, 0x0001FFFFFFFFFFFFULL,
    0x0000FFFFFFFFFFFFULL, 0x00007FFFFFFFFFFFULL, 0x00003FFFFFFFFFFFULL, 0x00001FFFFFFFFFFFULL,
    0x00000FFFFFFFFFFFULL, 0x000007FFFFFFFFFFULL, 0x000003FFFFFFFFFFULL, 0x000001FFFFFFFFFFULL,
    0x000000FFFFFFFFFFULL, 0x0000007FFFFFFFFFULL, 0x0000003FFFFFFFFFULL, 0x0000001FFFFFFFFFULL,
    0x0000000FFFFFFFFFULL, 0x00000007FFFFFFFFULL, 0x00000003FFFFFFFFULL, 0x00000001FFFFFFFFULL,
    0x00000000FFFFFFFFULL, 0x000000007FFFFFFFULL, 0x000000003FFFFFFFULL, 0x000000001FFFFFFFULL,
    0x000000000FFFFFFFULL, 0x0000000007FFFFFFULL, 0x0000000003FFFFFFULL, 0x0000000001FFFFFFULL,
    0x0000000000FFFFFFULL, 0x00000000007FFFFFULL, 0x00000000003FFFFFULL, 0x00000000001FFFFFULL,
    0x00000000000FFFFFULL, 0x000000000007FFFFULL, 0x000000000003FFFFULL, 0x000000000001FFFFULL,
    0x000000000000FFFFULL, 0x0000000000007FFFULL, 0x0000000000003FFFULL, 0x0000000000001FFFULL,
    0x0000000000000FFFULL, 0x00000000000007FFULL, 0x00000000000003FFULL, 0x00000000000001FFULL,
    0x00000000000000FFULL, 0x000000000000007FULL, 0x000000000000003FULL, 0x000000000000001FULL,
    0x000000000000000FULL, 0x0000000000000007ULL, 0x0000000000000003ULL, 0x0000000000000001ULL,
...
namespace u32
{
// Return a u32 mask with only the n lowest bits set.
static inline uint32_t only_low_bits_set(unsigned int n)
{
    // need to cast to u64 since shifting by 32 is undefined
    return (uint32_t)(((uint64_t)1 << n) - 1);
}

// Return a u32 mask with all bits set EXCEPT for the lowest n bits.
static inline uint32_t only_low_bits_unset(unsigned int n) { return (uint32_t)~(uint64_t)0 << n; }

// Return a u32 mask with only the n highest bits set.
static inline uint32_t only_high_bits_set(int n) { return only_low_bits_unset(32 - n); }

// Return a u32 mask with only the n highest bits unset.
static inline uint32_t only_high_bits_unset(int n) { return only_low_bits_set(32 - n); }
}
...
public:
    ...
    static uint8_t Popcount64bit(uint64_t n) { return (uint8_t)__builtin_popcountll(n); }

    static uint16_t Popcount512bit(uint64_t *data)
    {
        return (uint16_t)(__builtin_popcountll(*data) + __builtin_popcountll(*(data + 1)) +
...
public:
    ...
    // note the input should be a valid pointer
    static uint16_t Popcount512bit(uint64_t *data)
    {
        return (uint16_t)(__builtin_popcountll(*data) + __builtin_popcountll(*(data + 1)) +
                          __builtin_popcountll(*(data + 2)) + __builtin_popcountll(*(data + 3)) +
